...
/// What the menu bar shows. Either a calm green dot or an orange indicator
/// with a short abbreviation of the most recently active project.
...
/// - **running** — the same dotted circle in orange, followed by a 3-character
///   uppercase abbreviation of the most-recently running project (plus `·N`
///   when several projects are busy at once).
///
/// Every state is static on purpose. The label is on screen permanently and
/// each frame of any animation re-runs the whole MenuBarExtra layout pass, so
/// a spinner kept the app at several percent CPU for as long as a pipeline ran.
